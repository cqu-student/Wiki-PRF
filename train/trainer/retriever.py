...
from transformers import AutoModel, AutoProcessor, CLIPVisionModel, CLIPImageProcessor, AutoTokenizer, CLIPTokenizer
import faiss
import numpy as np
...
class WikipediaKnowledgeBase(KnowledgeBase):
    ...
    def load_knowledge_base(self, image_dict=None, scores_path=None, visual_attr=None):
        ...
        """
        try:
            # with jsonlines.open(self.knowledge_base_path, mode="r") as reader:
            #     return list(reader)
            with open(self.knowledge_base_path, "rb") as f:
                knowledge_base_dict = json.load(f)
        except:
...
class ClipRetriever(Retriever):
    ...
    def load_faiss_index(self, load_index_path, gpu_choice):
        ...
        if load_index_path is not None:
            ...
            self.faiss_index = faiss.index_cpu_to_gpu(res, gpu_choice, self.faiss_index)
    ...
    def similarity_section_image(self,image,texts):
        ...
        with torch.no_grad():
            ...
            # 提取文本特征（批量处理）
            text_inputs = processor(text=texts, padding=True, truncation=True, return_tensors="pt").to(self.device)
    ...
    def similarity_section_text(self,text_candidate,texts,top_k=5):
        ...
        with torch.no_grad():
            # 提取文本特征
            text_inputs_candidate = processor(text=[text_candidate], padding="max_length", truncation=True, max_length=77, return_tensors="pt").to(self.device)
            ...
            # 提取文本特征（批量处理）
            text_inputs = processor(text=texts, padding=True, truncation=True,max_length=77, return_tensors="pt").to(self.device)
            text_features = self.model.encode_text(**text_inputs)
            text_features /= text_features.norm(dim=-1, keepdim=True)  # 归一化
        # 计算相似度（等价Faiss内积）
